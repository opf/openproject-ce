...
describe UsersController, type: :controller do
  ...
  describe "GET /users/:id/dump_avatar" do
    ...
    let(:action) { get :dump_avatar, params: { id: user.id } }
    ...
    context "for an invalid user" do
      let(:do_action) { get :dump_avatar, params: { id: 0} }
    ...
    context "for a user without an avatar" do
      ...
      let(:do_action) { get :dump_avatar, params: { id: user.id} }
      it_should_behave_like "an action with an invalid user"
    end
    #
    context "for a user with an avatar" do
      ...
      let(:do_action) { get :dump_avatar, params: { id: user.id } }
      it {
        # expect(@controller).to receive(:send_file).and_return true
        do_action
  ...
  describe "POST /users/:id/update_avatar" do
    let(:user) { user_with_avatar }
    let(:action) { post :update_avatar, params: { avatar: avatar_file, id: user.id } }
    ...
    context "WHEN save submit" do
      ...
      context "for an invalid user" do
        let(:do_action) { post :update_avatar, params: submit_param.merge(id: 0)}
        it_should_behave_like "an action with an invalid user"
      ...
      context "for a user without an avatar" do
        let(:user) { user_without_avatar }
        it_should_behave_like "an action with stubbed User.find"
        let(:do_action) { post :update_avatar, params: submit_param.merge(id: user.id)}
